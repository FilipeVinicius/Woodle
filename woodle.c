...
void sortearPalavra(char dicionario_palavras[5][6], char sortear_palavra[6]);
void finalJogada(char palavra_sorteada[6], int contador_tentativas, double tempo_decorrido);
void jogo(char dicionario_palavras[5][6], char palavra_sorteada[6]);
double calcularTempoDecorrido(time_t inicio);

int main()
{
    // Lógica do sorteio das palavras
    char dicionario_palavras[5][6] = {"areia", "areio", "casar", "parar", "comer"}; // Palavras de teste
...
void sortearPalavra(char dicionario_palavras[5][6], char sortear_palavra[6])
{
    int indice = rand() % 5; // gerar um indice aleatório entre 0 e 5
    strcpy(sortear_palavra, dicionario_palavras[indice]);
}
